...
public enum MatchEngine {
    ...
    public static func cpuTeamSkills<R: RandomNumberGenerator>(
        ...
        // Zufall ausschliesslich ueber die `engineRandom*`-Helfer: nur die
        // gibt es auf beiden Seiten des `#if SKIP`-Splits (`engineNextBounded`
        // existiert nur unter Skip).
        ...
        case .dfb, .uefa:
            g[0] = 53 + Int(Double(posF) * 1.5)
                 + engineRandomInt(0..<max(1, runde * 4), &rng)
                 + runde * 6
        case .liga:
            let raw = 10 + posF * 4 + engineRandomInt(0..<max(1, pos16 * 2), &rng)
            g[0] = Int(Double(raw) * 1.4)
        }

        let zrRaw = 25 + posF * 2 + engineRandomInt(0..<max(1, pos16 * 3), &rng)
        g[1] = Int(Double(zrRaw) * 1.4)

        g[2] = 60 + engineRandomInt(0..<80, &rng)
        ...
        for _ in 0..<iterations {
            ...
            var tries = 0
            while tries < 20 {
                let a = engineRandomIntClosed(3, 6, &rng)
                if g[a] < 160 {
                    g[a] += 6
                    break
                }
