...
///
/// SKIP-FALLE (2026-09-22): dieser Typ stand zuerst als verschachteltes Enum
/// IN der `extension MatchEngine`. Kotlin kann einem Typ per Extension keinen
/// verschachtelten Typ hinzufuegen, skipstone brach mit „'CPUSkillContext' is
/// not a member type of enum 'MatchEngine.MatchEngine'" ab. Verschachtelte
/// Typen gehoeren in die Original-Deklaration oder — wie hier — auf
/// Modulebene.
...
public enum MatchEngine {
    ...
    // MARK: - CPU-Gegner-Skills (KICKER.BAS:1536-1560)
    //
    // SKIP-FALLE (2026-09-22): Diese beiden Funktionen standen zuerst in
    // einer eigenen Datei als `extension MatchEngine`. Der Transpiler legt
    // die Kotlin-Datei zwar an, nimmt die statischen Member einer Extension
    // aber NICHT in die `MatchEngine.skipcode.json` auf — game-core brach
    // dann mit „type 'MatchEngine' has no member 'cpuTeamSkills'" ab,
    // obwohl iOS und das Backend sauber bauten. Statische Member eines
    // Enums gehoeren in den Rumpf der Original-Deklaration.
    ...
    public static func cpuTeamSkills<R: RandomNumberGenerator>(
        ...
        // ist die Skip-freie Linux-Fassung und hat die `engineRandom*`-Helfer
        // nicht — hier steht `Int.random(in:using:)` direkt. Wer die Formel
        // aendert, muss BEIDE Zweige anfassen.
    ...
    //
    // Linux-Fassung dieses Zweigs: kein Skip, deshalb `Int.random(in:using:)`
    // statt der `engineRandom*`-Helfer. Die FORMEL ist identisch zu `main` —
    // wer sie hier aendert, aendert sie dort mit, sonst laufen Solo und Online
    // auseinander.
    ...
    private static func knockoutPenalties<R: RandomNumberGenerator>(
        ...
        while (hTaken < maxRegular || aTaken < maxRegular) && !decided {
            if hTaken < maxRegular {
                if Int.random(in: 0..<4, using: &rng) > 0 { home += 1 }
                hTaken += 1
            }
            if !decided && aTaken < maxRegular {
                if Int.random(in: 0..<4, using: &rng) > 0 { away += 1 }
                aTaken += 1
            }
        ...
        while home == away {
            let h = Int.random(in: 0..<4, using: &rng) > 0
            let a = Int.random(in: 0..<4, using: &rng) > 0
            if h { home += 1 }
            if a { away += 1 }
        }
